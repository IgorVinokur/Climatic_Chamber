// конструктор WEB страницыupdate
void build() {
  GP.BUILD_BEGIN(700);
  GP.THEME(GP_DARK);
  GP.ICON_SUPPORT();
  ...
  if (ui.uri() == "/cfg") {
    ...
    GP_MAKE_FORM(
      "/cfg", "SAVE",
      GP.RELOAD_CLICK("sw_temp,sw_humidity,sw_draining,sw_a_circulation,sw_ventilation,sw_q_lamp,sw_mqtt");
      GP_MAKE_GRID(
        GP_MAKE_BLOCK_THIN_TAB(
          ...
          GP.SWITCH("sw_temp", mydata.sw_temp););
    ...
    GP_MAKE_FORM(
      ...
      GP_MAKE_BLOCK_TAB(
        ...
        GP.SWITCH("sw_mqtt", mydata.sw_mqtt);););
